...
var meow = require("meow"), errors = require("@changesets/errors"), logger = require("@changesets/logger"), util = require("util"), fs = require("fs-extra"), path = require("path"), getPackages = require("@manypkg/get-packages"), getDependentsGraph = require("@changesets/get-dependents-graph"), config = require("@changesets/config"), chalk = require("chalk"), child_process = require("child_process"), termSize = require("term-size"), enquirer = require("enquirer"), externalEditor = require("external-editor"), ansiColors = require("ansi-colors"), git = require("@changesets/git"), writeChangeset = require("@changesets/write"), resolveFrom = require("resolve-from"), semverLt = require("semver/functions/lt"), outdent = require("outdent"), applyReleasePlan = require("@changesets/apply-release-plan"), readChangesets = require("@changesets/read"), assembleReleasePlan = require("@changesets/assemble-release-plan"), pre$1 = require("@changesets/pre"), semverParse = require("semver/functions/parse"), pLimit = require("p-limit"), preferredPM = require("preferred-pm"), spawn = require("spawndamnit"), isCI = require("is-ci"), table = require("tty-table"), getReleasePlan = require("@changesets/get-release-plan");
...
async function getPublishTool(cwd) {
  ...
  try {
    let version = (await spawn__default.default("pnpm", [ "--version" ], {
      cwd: cwd
    })).stdout.toString().trim(), parsed = semverParse__default.default(version);
    return {
      name: "pnpm",
...
async function getUnpublishedPackages(packages, preState) {
  const results = await Promise.all(packages.map((async ({packageJson: packageJson}) => {
    ...
    return response.published && (publishedState = "published", void 0 !== preState && response.pkgInfo.versions && response.pkgInfo.versions.every((version => semverParse__default.default(version).prerelease[0] === preState.tag)) && (publishedState = "only-pre")), 
    {
